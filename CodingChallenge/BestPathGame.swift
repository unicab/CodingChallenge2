...
public class BestPathGame {
    fileprivate let matrix: [[Any]]?

    public init(forMatrix matrix: [[Any]]?) {
        self.matrix = matrix
    }
    
    public func computeBestPath() throws -> (SumaryInfo, Bool)? {
        guard self.matrix != nil else { throw MatrixError.emptyMatrix }
        return try computeBestPath(forInput: self.matrix)
    }
}

extension BestPathGame {
    
    // compute best path for a matrix
    
    fileprivate func computeBestPath(forInput input: [[Any]]?) throws -> (SumaryInfo, Bool)? {
